...
Every snapshot includes a conservative `visibility` field. Native AX results
use `scope: current_accessibility_view` and captured results use
`scope: captured_window`; both mark `may_contain_scrolled_out: true` and
`complete: false`, because absence cannot prove that a control does not exist.
Scroll and observe again before concluding that a control is missing.
